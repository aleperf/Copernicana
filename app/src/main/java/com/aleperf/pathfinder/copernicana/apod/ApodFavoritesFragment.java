...
public class ApodFavoritesFragment extends Fragment {
    ...
    MutableLiveData<List<Apod>> lastSearchedApod;
    LiveData<List<Apod>> first30Apods;
    LiveData<List<Apod>> last30Apods;
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        //
        first30Apods = favoritesViewModel.loadFavoritesApod();
        subscribeAllFavorites();
        subscribeFirst30Apods();

    }

    private void subscribeFirst30Apods(){
        Observer<List<Apod>> first30ApodsObserver = new Observer<List<Apod>>() {
            @Override
            public void onChanged(@Nullable List<Apod> apodList) {
                if(apodList != null && apodList.size() > 0){
                   //apodAdapter.setApod(apodList);
                    allFavoritesApods.setValue(apodList);
                }
            }
        };
        first30Apods.observe(this, first30ApodsObserver);
    }
